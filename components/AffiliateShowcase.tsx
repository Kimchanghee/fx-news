...
function getOffers(locale: Locale): Offer[] {
  ...
  const coupangSearch = `https://www.coupang.com/np/search?component=&q=${encodeURIComponent('외환 트레이딩 도서')}`;

  // AliExpress search URL — fallback to trader gadget category
  const aliExpressSearch = 'https://www.aliexpress.com/w/wholesale-trading-monitor-stand.html';

  const amazonUrl = new URL('https://www.amazon.com/s');
  amazonUrl.searchParams.set('k', 'forex trading book');
